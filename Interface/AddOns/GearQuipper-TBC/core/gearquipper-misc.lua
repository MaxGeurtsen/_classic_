	...
	if itemString ~= c.VALUE_NONE then
		for index, setName in ipairs(c:LoadSetNames()) do
			for slotId in pairs(c:GetSlotInfo()) do
				if (not c:LoadPartialOption(setName) or c:LoadSlotState(slotId, setName)) and c:LoadSlot(slotId, setName) == itemString then
...
function c:GetZoneChildrenSorted(parentMapId)
	if not zoneCache[parentMapId] then
		...
		table.sort(zoneCache[parentMapId],
			function(a, b)
...
function c:ExperimentalCanUseStuff()
	...
	MyAddonTooltip:AddFontStrings(
		MyAddonTooltip:CreateFontString( "$parentTextLeft1", nil, "GameTooltipText" ),
