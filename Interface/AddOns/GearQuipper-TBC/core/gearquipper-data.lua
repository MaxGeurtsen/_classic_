...
function c:BindingExists(binding)
    if binding[c.FIELD_TYPE] then
        ...
        for index, matchedBinding in pairs(c:LoadEventBindings(bindingClone)) do
            if (binding[c.FIELD_PVE] and matchedBinding[c.FIELD_PVE]) or
                (binding[c.FIELD_PVP] and matchedBinding[c.FIELD_PVP]) then
                return true;
            end
        end
    end
    ...
    if setName and GQ_DATA[c:GetRealmName()][c:GetCharName()][FIELD_SETS][setName] then
        ...
        if notify then
            c:NotifyInChat(string.format("<%s> %s %s", setName, c:GetText("Affects helmet visibility:"),
                c:BoolToText(value, c:GetText("Yes"), c:GetText("No"))));
        end
    end
end
